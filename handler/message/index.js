...
*/
        ...
        case 'gifsticker': {
            ...
            if (isGiphy) {
                ...
                await client.reply(from, 'Hanya bisa membuat sticker dari Giphy', id)
            }
            break
        }
        // Video Downloader //
        ...
        case 'ig':
                if (args.length !== 1) return client.reply(from, 'Maaf, format pesan salah silahkan periksa menu. [Wrong Format]', id)
                if (!isUrl(url) && !url.includes('instagram.com')) return client.reply(from, 'Maaf, link yang kamu kirim tidak valid. [Invalid Link]', id)
                client.reply(from, '_Scraping Metadata...\n\nUntuk Donasi Ketik *#donasi*', id)
        ...
        case 'tts':
         if (!isGroupMsg) return client.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!\nSilahkan invite ke grup terlebih dahulu', message.id)
            ...
            if (dataText === '') return client.reply(from, 'Kode Bahasa yang digunakan salah', message.id)
            if (dataText.length > 1000) return client.reply(from, 'Teks tidak boleh terlalu panjang', message.id)
        ...
        case 'leaveall':
            ...
            client.reply(from, 'Success leave all group!', message.id)
            break
        case 'clearall':
            if (!isOwner) return client.reply(from, 'Perintah ini hanya untuk Owner bot', message.id)
            ...
            client.reply(from, 'Success clear all chat!', message.id)
            break
        case 'bc':
            if(!isOwner) return client.reply(from, 'Perintah ini hanya untuk owner bot!', message.id)
            ...
            for (let ids of chatz) {
                ...
                if (!cvk.isReadOnly) client.sendText(ids, `*[ PURTA Broadcast ]*\n\n${msg}`)
            }
...
break
        ...
        case 'randomanime':
            ...
            case 'fetish':
                ...
                if (!request) {
                    client.reply(from, 'Yang tersedia: *armpits*, *feets*, *thighs*, *booty*, *boobs*, *necks*, *belly*, *sideboobs*, dan *ahegao*')
                }
                client.reply(from, '_Sedang mencari..._', id)
